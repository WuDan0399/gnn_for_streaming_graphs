...
condition_key2id = {"add_only":0, "del_no_change":1, "covered":2, "recompute":3}
...
def add_mask(data: pyg.data.Data) -> None:
    if not hasattr(data, "train_mask"):  # add train\test\validation masks if not any
        ...
        data.val_mask = torch.tensor(
            [True if x >= train_ratio + test_ratio else False for x in rand_choice],
            dtype=torch.bool,
        )
...
def general_parser(parser: argparse.ArgumentParser) -> argparse.Namespace:
    ...
    parser.add_argument("-l", "--nlayers", default=5, type=int, help="number of layers")
    parser.add_argument(
        "-i",
        "--initial",
        default=70.0,
