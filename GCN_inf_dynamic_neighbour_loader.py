...
def main():
    ...
    if len(available_model) == 0 :  # no available model, train from scratch
        ...
        ## run inference for whole graph (edge added). save the intermediate value for each layer and each node as the final targeted result.
        ...
        sample_edges, initial_edges = random_edge_remove(data.edge_index.numpy(), batch_size, data.is_directed())
        ...
        ## calculate theorectially affected number of nodes
        ...
        ## run inference for the initial graph (before edge adding). save rthe intermediate value for each layer and each node as the initial result
        ...
        ## load the two files\arrays and compare. calculate different rate at number level and node level. Different rate indicates the affected area (value changed)
